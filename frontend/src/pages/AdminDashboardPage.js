...
const AdminDashboardPage = () => {
    ...
    const handleUpdateItem = async (id, formData) => {
        try {
            ...
            switch (activeTab) {
                case 'events':
                    response = await axiosInstance.put(`/events/${id}`, formData);
                    break;
                case 'venues':
                    response = await axiosInstance.put(`/venues/${id}`, formData);
                    break;
                case 'users':
                    response = await axiosInstance.put(`/users/${id}`, formData);
                    break;
                default:
                    break;
            }
                        ...
                        <Table striped hover>
                            ...
                            {tickets.map(ticket => (
                                <tr key={ticket.id}>
                                    ...
                                    <td>${ticket.currentPrice?.toFixed(2)}</td>
                                    <td>
                                        {ticket.owner ?
        ...
        useEffect(() => {
            if (formData.eventId) {
                ...
                const fetchPricingTiers = async () => {
                    try {
                        const response = await axiosInstance.get(`/pricing-tiers/event/${formData.eventId}`);
